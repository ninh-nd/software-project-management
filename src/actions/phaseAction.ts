...
export async function addTaskToPhase(phaseId: string, taskId: string): PromiseServer<IPhase> {
    const response = await resourcesAPI.patch(`/phase/${phaseId}/task/add`, { taskId })
    return response.data
}
export async function removeTaskFromPhase(phaseId: string, taskId: string): PromiseServer<IPhase> {
